...
class TransformScanNet:
  ...
  def transform_scannet(self, sample, idx):
    ...
    if self.flags.limit:
      filename = os.path.splitext(os.path.basename(self.filenames[idx]))[0]
      limit_idx = torch.from_numpy(self.lim_idx[filename])
      return points, limit_idx
  ...
  def __call__(self, sample, idx=None):
    if self.flags.limit:
      points, lim = self.transform_scannet(sample, idx)
      points, inbox_mask = ocnn.TransformPoints(**self.flags)(points)
      octree = ocnn.Points2Octree(**self.flags)(points)
      return {'octree': octree, 'points': points, 'inbox_mask': inbox_mask, 'limit': lim}
    
    elif 'limited_reconst' in self.flags:
      # transformation specified for scannet
      points = self.transform_scannet(sample, idx)
      ...
      octree = ocnn.Points2Octree(**self.flags)(points)
      return {'octre': octree, 'octree': octree, 'points': points, 'inbox_mask': inbox_mask}
