...
def read_file(filename):
  def _read_ply(filename):
    ...
    label = np.asarray(vtx['mask']).astype(np.float32)
...
class TransformScanNet:
  def __init__(self, flags):
    ...
    self.img_lst = []
    for idx, line in enumerate(open(flags.filelist.split('.')[0] + '_img.txt').readlines()):
        self.img_lst.append(os.path.join(flags.location, line.split('\n')[0]))

  def transform_scannet(self, sample, idx):
    # read ply file
    # xyz, color, label, normal = read_file(filename)
    xyz, color, mask, normal = sample
    label = np.full(mask.shape, 0, dtype='float32')
  ...
  def __call__(self, sample, idx=None):
    ...
    points, mask = self.transform_scannet(sample, idx)
    #softlabel = get_softlabel(self.img_lst[idx], mask)
    ...
    return {'octree': octree, 'points': points, 'inbox_mask': inbox_mask, 'softmask': mask, 'img': imread(self.img_lst[idx])}


def get_mp3d_dataset(flags):
  transform = TransformScanNet(flags)
  dataset = Dataset(flags.location, flags.filelist, transform,
                    read_file=read_file, in_memory=flags.in_memory)
  transform.filenames = dataset.filenames
