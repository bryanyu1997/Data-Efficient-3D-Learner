...
from datasets import get_mp3d_dataset

from tools import DPTSegmentationModel, Resize, NormalizeImage, PrepareForNet
...
def loss_function(logit, label):
  ...
  return torch.sum(torch.mul(log_likelihood, label), dim=1).mean()
...
class SegSolver(Solver):
  def get_model(self, flags):
    self.model_seg = DPTSegmentationModel(
            150,
            path="tools/DPT/weights/dpt_hybrid-ade20k-53898607.pt",
            backbone="vitb_rn50_384",
        )
    self.model_seg.cuda(device=self.device)
  ...
  def get_dataset(self, flags):
    if flags.name.lower() == 'mp3d':
      return get_mp3d_dataset(flags)
    else:
  ...
  def get_softlabel(self, batch_img, batch_somask, batch_inmask):
    soft_target = []
    for i in range(len(batch_img)):
      image = batch_img[i]
      mask = batch_somask[i]
      inbox_mask = batch_inmask[i].bool()
      with torch.no_grad():
        self.model_seg.eval()
        img = self.transform_seg({"image": image / 255.})["image"]
        img = torch.from_numpy(img).cuda().unsqueeze(0)
        pred = self.model_seg(img)
        pred = torch.nn.functional.interpolate(pred, size=image.shape[:2], mode="bicubic", align_corners=False).squeeze()
        pred = F.softmax(pred, dim=0)
        pred = pred.reshape(150, -1)[:, mask].permute(1, 0)
        pred = pred[inbox_mask]
        soft_target.append(pred)
    soft_target = torch.cat(soft_target, 0)
  ...
  def model_forward(self, batch):
    ...
    softlabel = self.get_softlabel(batch['img'], batch['softmask'], batch['inbox_mask'])
    logit = self.model(octree, pts)

    return logit, softlabel

  def train_step(self, batch):
    logit, softlabel = self.model_forward(batch)
    loss = loss_function(logit, softlabel)
