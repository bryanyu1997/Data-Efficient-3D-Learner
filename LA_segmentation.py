...
class SegSolver(Solver):
  ...
  def model_forward(self, batch):
    ...
    if self.model.training:
      if self.FLAGS.DATA.train.semi:
        ...
        pred_l, pred_u, targ, pseu = [], [], [], []
        for i in range(self.FLAGS.DATA.train.batch_size):
          mm = batch['inbox_mask'][i]
          ll = batch['limit'][i]
          mi = mm.nonzero()
          map = torch.zeros(mm.shape[0])
          map[ll] = 1 
          mask_l = map[mi].squeeze().bool()
          mask_u = (map[mi]==0).squeeze().bool()

          pred_l.append(logit[pts[:, 3]==i][mask_l])
          targ.append(label[pts[:, 3]==i][mask_l])
          pred_u.append(logit[pts[:, 3]==i][mask_u])
          pseu.append(logit_stdnt[pts[:, 3]==i][mask_u])

        logit_l = torch.cat(pred_l, 0)
        label = torch.cat(targ, 0)
        logit_u = torch.cat(pred_u, 0)
        logit_u_stdnt = torch.cat(pseu, 0)
  ...
  def train_step(self, batch):
    if self.FLAGS.DATA.train.semi:
      ...
      loss_total = 0 
      loss['label'] = 1 * loss_function(logit, label)

      consist_weight = get_current_consistency_weight(batch['epoch'], self.FLAGS.LOSS.consistency, self.FLAGS.LOSS.consist_rampup)
      loss['unlabel_consist'] = consist_weight * loss_function_consist(unlbl_logit, unlbl_logit_stdnt)
      loss['unlabel_mini_entpy'] = self.FLAGS.LOSS.weight_sum * loss_function_mini_enpty(unlbl_logit)
      self.global_step += 1
        
      for k, v in loss.items():
        loss_total += v if not torch.isnan(v) else 0 
